...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
        ...
        ay = a[y[i]]
        a_n = np.delete(a,y[i])
        loss += np.sum(np.maximum(0,(1+a_n - ay)))
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
        ...
        ay = a[y[i]]
        a_n = np.delete(a,y[i])
        z = np.maximum(0,(1+a_n - ay))
        loss += np.sum(z)
        z_ind = np.sign(z)
        z_ind = z_ind.reshape((np.shape(z_ind)[0],1))
        grad_tmp1 = np.multiply(X[i],z_ind)
        grad_tmp2 = -1*np.sum(grad_tmp1,axis=0)
        grad += np.insert(grad_tmp1,y[i],grad_tmp2,axis=0)
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    ay = a[y,ind]
    z = np.maximum(0,(1+a - ay))
    loss_arr = np.sum(z,axis=0)
    loss = np.mean(loss_arr) - 1
    ...
    # ================================================================ #
    z_ind = np.sign(z)
    z_inds = np.sum(z_ind,axis=0)
    z_ind[y,ind] = (-1*z_inds)+1
    grad = z_ind.dot(X)
    grad /= num_train 
