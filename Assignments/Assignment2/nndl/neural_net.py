...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # ================================================================ #
    a1 = W1.dot(X.T) + b1.reshape((b1.shape[0],1))
    h1 = np.maximum(0,a1)
    scores = W2.dot(h1) + b2.reshape((b2.shape[0],1))
    scores = scores.T
    ...
    ay = a[y,ind]
    lgk = -1*np.amax(a,axis=0)
    zexp = np.exp(a+lgk)
    zsum = np.sum(zexp,axis=0)
    loss_arr = np.log(zsum) - ay -lgk
    loss = np.mean(loss_arr)
    l2_loss = 0.5*reg*(np.sum(np.square(W1)) + np.sum(np.square(W2)))
    loss += l2_loss 
    ...
    db2 = np.divide(zexp,zsum)
    db2[y,ind] -= 1
    dW2 = db2.dot(h1.T)
    DW2 = dW2/N
    Db2 = db2.sum(axis=1)/N
    dh1 = (W2.T).dot(db2)
    da1 = np.multiply(np.sign(np.maximum(0,a1)),dh1)
    db1 = da1
    Db1 = db1.sum(axis=1)/N
    dW1 = da1.dot(X)
    DW1 = dW1/N
    grads['W1']=DW1 + reg*W1
    grads['b1'] = Db1
