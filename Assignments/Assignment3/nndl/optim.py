...
def rmsprop(w, dw, config=None):
  ...
  # ================================================================ #
    ...
    e = config['learning_rate']
    a = config['a']
    a = b*a + (1-b)*np.multiply(dw,dw)
    next_w = w - e*np.divide(dw,(np.sqrt(a)+mu))
    config['a'] = a
...
def adam(w, dw, config=None):
  ...
  # ================================================================ #
    ...
    t = t+1
    v = b1*v + (1-b1)*dw
    a = b2*a + (1-b2)*np.multiply(dw,dw)
    vtil = (1/(1- b1**t))*v
    atil = (1/(1- b2**t))*a
    next_w = w - e*np.divide(vtil,(np.sqrt(atil)+mu)) 
    config['t'] = t
    config['a'] = a
    config['v'] = v
