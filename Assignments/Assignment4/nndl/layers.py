...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  """
    ...
    # ================================================================ #
        ...
        running_mean = momentum*running_mean + (1-momentum)*sample_mean
        running_var = momentum*running_var + (1-momentum)*sample_var
        xn = np.divide((x-sample_mean),(np.sqrt(sample_var+eps)))
        out = gamma*xn + beta
        cache = (x,xn,gamma,sample_mean,sample_var,eps)
    ...
    # ================================================================ #
        ...
        xn = np.divide((x-running_mean),(np.sqrt(running_var+eps)))
        out = gamma*xn + beta
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
    x,xn,gamma,sample_mean,sample_var,eps = cache
    N = x.shape[0]
    dbeta = np.sum(dout,axis=0)
    dgamma = np.sum((np.multiply(dout,xn)),axis=0)
    dxn = np.multiply(dout,gamma)
    dvar = -0.5*np.divide(np.sum(np.multiply((x-sample_mean),dxn),axis=0),((sample_var+eps)**1.5))
    dmu = -1*np.divide(np.sum(dxn,axis=0),np.sqrt(sample_var+eps))
    dx = np.divide(dxn,np.sqrt(sample_var+eps)) + (2/N)*np.multiply((x-sample_mean),dvar) + dmu/N
